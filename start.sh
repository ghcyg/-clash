...
if [ "$arch" = "x86_64" ]; then
    echo 正在启动64位clash
    cd /root/clash/bin && nohup ./clash-linux-amd64 -f Customization.yaml > /root/clash/log.txt 2>&1 &
elif [[ "$arch" =~ "aarch64" ||  "$arch" =~ "arm64" ]]; then
    echo 正在启动32位程序
    cd /root/clash/bin && nohup ./clash-linux-arm64 -f Customization.yaml > /root/clash/log.txt 2>&1 &
elif [ "$arch" = "armv7" ]; then
    echo 正在启动32位程序
    # 在此处添加执行 32 位程序的命令    
    echo 正在启动32位程序
    cd /root/clash/bin && nohup ./clash-linux-armv7 -f Customization.yaml > /root/clash/log.txt 2>&1 &
else
    echo "退出"
